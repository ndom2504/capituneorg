...
if (!connectionString) {
  ...
  console.error(
    "Tip: create an env file (uncommitted) containing DIRECT_URL=... and run: node scripts/apply-platform-settings-migration.mjs --env-file .env.neon.staging",
  );
  process.exitCode = 1;
  process.exit();
}
...
async function main() {
  ...
  console.log(`Starting platform settings migration${targetLabel}...`);

  await sql`SELECT 1 as ok`;

  const tableExistsBefore = await sql`SELECT to_regclass('public."PlatformSetting"') as reg`;
  const regBefore = Array.isArray(tableExistsBefore) ? tableExistsBefore[0]?.reg ?? null : null;
  console.log(`PlatformSetting regclass before${targetLabel}:`, regBefore);

  if (regBefore) {
    console.log(`PlatformSetting already exists${targetLabel}; skipping apply.`);
  } else {
  ...
  const tableExistsAfter = await sql`SELECT to_regclass('public."PlatformSetting"') as reg`;
  const regAfter = Array.isArray(tableExistsAfter) ? tableExistsAfter[0]?.reg ?? null : null;
  console.log(`PlatformSetting regclass after${targetLabel}:`, regAfter);

  if (!regAfter) {
    console.error("Migration ran, but PlatformSetting still missing.");
    process.exitCode = 1;
    return;
  }
  ...
  if (!set.has("UPDATE_PLATFORM_SETTING")) {
    console.error("Migration applied but UPDATE_PLATFORM_SETTING is missing from AuditAction.");
    process.exitCode = 1;
    return;
  }

  console.log(`OK${targetLabel}: PlatformSetting table exists and AuditAction contains UPDATE_PLATFORM_SETTING.`);
}
